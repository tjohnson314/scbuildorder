...
CMemoryPoolManager CFastString::m_memoryPoolManager;
...
{
	if(m_data)
		m_memoryPoolManager.GetMemoryPool(m_capacity)->Free(m_data);
}
...
void CFastString::capacity(size_t size)
{
	...
	char *newData = (char *)m_memoryPoolManager.GetMemoryPoolAddAsNeeded(newCapacity)->Alloc();
	if(m_data)
	{
